...
import com.lukaszfabia.ferby.core.result.FerbyResult.Failure
import com.lukaszfabia.ferby.core.result.FerbyResult.Success
...
suspend fun <T, R> FerbyResult<T>.flatMap(transform: suspend (T) -> FerbyResult<R>): FerbyResult<R> =
    when (this) {
...
suspend fun <T> FerbyResult<T?>.ifNull(fallback: suspend () -> FerbyResult<T>): FerbyResult<T> =
    this.flatMap {
        if (it == null) {
            fallback()
        } else {
